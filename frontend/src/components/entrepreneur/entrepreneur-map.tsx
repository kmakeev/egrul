...
interface EntrepreneurMapProps {
  address: Address;
...
export function EntrepreneurMap({ address, entrepreneurName = 'ИП' }: EntrepreneurMapProps) {
  ...
  // Геокодирование по текстовому адресу
  ...
  const initMap = useCallback(() => {
    ...
    const placemark = new window.ymaps.Placemark(coordinates, {
      balloonContent: `
        <div style="padding: 8px;">
          <strong>${entrepreneurName}</strong><br/>
          <small>${memoizedAddress.fullAddress || 'Адрес не указан'}</small>
        </div>
      `,
      hintContent: entrepreneurName
    }, {
    ...
    setMapInstance(map);
  }, [coordinates, entrepreneurName, memoizedAddress.fullAddress]);
  ...
  const openInYandexMaps = () => {
    if (coordinates) {
      ...
      window.open(`https://yandex.ru/maps/?text=${encodeURIComponent(address.fullAddress)}`, '_blank');
    }
  };
